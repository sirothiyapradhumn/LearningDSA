...
public class Main {
  ...
  public static void solution(String unique, int idx, HashMap<Character, Integer> map, boolean[] usedNumbers, String s1, String s2, String s3) {
	    // write your code here
      //base case
      if(idx == unique.length()){
        if((getValue(s1, map)+ getValue(s2,map)) == getValue(s3, map)){
          
          //for loop for print in lexicol order
          for(int i =0; i<26; i++){
            char ch = (char)(i+'a');
